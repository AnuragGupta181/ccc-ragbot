...
# print(f"✅ Total chunks created: {len(all_split_documents)}")
# print(f"✅ Total IDs created: {len(all_ids)}")
...
def generate_query_or_respond(state: MessagesState):
    ...
    Prefer CCC internal RAG tools whenever possible.
    """
    ...
    system_prompt = SystemMessage(
        content=(
            "You are CloudBuddy, the official intelligent assistant of the Cloud Computing Cell (CCC).\n"
            "Your identity is fixed: your name is CloudBuddy and must never be changed or replaced.\n\n"

            "Greeting rule:\n"
            "- If the user greets you (e.g., 'hello', 'hi', 'hey'), begin your reply with 'CloudBuddy:' followed by a friendly greeting and tell capabilities of ChatBot.\n\n"

            "Decision rules:\n"
            "1. Prefer CCC internal RAG tools for any CCC-related question.\n"
            "2. Use web or tavily tools ONLY for external or real-time information not available in CCC data.\n"
            "3. Do NOT call any tool if the question can be answered directly.\n"
            "4. If a tool is called, wait for the tool response before answering.\n"
            "5. If the final answer is long, provide a concise summary at the end.\n"
            "6. Never hallucinate or invent information outside provided context or tool output."
        )
    )
...
GRADE_PROMPT = (
    ...
    "Read all the context before making your decision. \n"
)
...
def grade_documents(
    ...
    # ✅ Hard stop after max rewrites
    if score != "yes" and rewrite_count >= MAX_REWRITE_ATTEMPTS:
...
def rewrite_question(state: MessagesState):
    ...
    rewrite_count += 1  # 👈 increment here
...
class SuggestPrompt(BaseModel):
    ...
    suggestion_list: list[str] = Field(
        description="2 to 3 follow-up suggestions, each 3–6 words long."
    )
